...
在 AmritaBot 框架中，提示词（Prompt）是引导大型语言模型（LLM）行为的关键因素。良好的提示词设计能够显著提升模型的响应质量和任务执行效率。本文档将详细介绍在 AmritaBot 框架中设计和使用提示词的最佳实践。

## 1. 提示词基础概念

提示词是提供给语言模型的初始文本输入，用于设定对话的背景、角色和任务目标。在 AmritaBot 框架中，提示词分为群聊提示词和私聊提示词两种类型，分别用于不同的对话场景。
...
AmritaBot 支持创建和管理多个提示词预设。在 WebUI 的「聊天管理 → 提示词预设」页面可以进行完整的 CRUD 操作。
...
提示词中可使用以下占位符，AmritaBot 在发送给 LLM 前会自动替换：

- `{cookie}` — Cookie 反注入检测字符串
- `{self_id}` — 机器人 QQ 号
- `{user_id}` — 当前用户 QQ 号
- `{user_name}` — 当前用户昵称
- `${环境变量名}` — 读取 `.env` 中的环境变量值
...
良好的提示词设计是发挥大型语言模型潜力的关键。在 AmritaBot 框架中，通过合理设计和管理提示词，可以显著提升模型的表现和用户体验。设计提示词时应遵循明确性、完整性、结构化等原则，并根据实际使用效果不断优化。

通过充分利用 AmritaBot 提供的提示词管理功能，开发者和用户可以创建出更加高效和个性化的对话体验。
